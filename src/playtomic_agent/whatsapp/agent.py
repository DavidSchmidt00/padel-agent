...
def _build_system_prompt(
    ...
    return (
        ...
        + (
            ...
            f"- ALWAYS set respond.{voting_field} whenever you have 2+ slot options in a group.\n"
            f"- NEVER list slots as plain text in a group — always use respond.{voting_field}.\n"
            f"- Slot sources:\n"
            f"  * find_slots → response.slots[] contains slot dicts — use them directly.\n"
            f"  * find_slots_date_range → response.results[i].slots[] contains slot dicts per day.\n"
            f"    Pick one representative slot per day and flatten into a single list.\n"
            f"- Copy each slot dict into respond.{voting_field}.slots"
            " (ALL fields required: display, date, local_time, court, court_type, duration, price, booking_link).\n"
            ...
            f"- SELF-CHECK before calling respond: if you have 2+ slot options in this group,"
            f" confirm respond.{voting_field} is set with at least 2 slots."
            f" If it is not set, add it now — do NOT call respond with only text_parts.\n\n"
            if is_group
            else ""
        )
        + "PREFERENCES:\n"
